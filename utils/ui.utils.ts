...
export class UIUtil {
  ...
  // Assertions
  async assertVisible(target: Locator, timeout = 5000) {
    await expect(this.loc(target)).toBeVisible({ timeout });
  }

  async assertTitle(target: string | RegExp, timeout = 5000) {
    await expect(this.page).toHaveTitle(target, { timeout });
  }
  ...
  async assertTextVisible(text: string | RegExp, timeout: number = 5000): Promise<void> {
    const locator = this.page.getByText(text);
    await expect(locator.first()).toBeVisible({ timeout });
  }

  async assertElementHasText(target: Locator | string, expectedText: string | RegExp): Promise<void> {
  ...
  getRandomNumber(min: number = 1000, max: number = 9999): number {
    return Math.floor(Math.random() * (max - min + 1)) + min;
  }

  getRandomEmployeeId(): string {
